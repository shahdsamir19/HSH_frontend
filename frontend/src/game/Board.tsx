...
export function Board() {
  const { clues, setPhase, setBoardScores, elapsedMs, pushToast, submitDetectiveReport } = useGame();
  ...
  // Listen to validation results from the server
  import_react_useEffect(() => {
    const handleReportResult = (e: any) => {
      if (e.detail && !e.detail.correct) {
        setErrorMsg(e.detail.message);
        pushToast({ tone: "warn", text: "Submission Rejected", sub: e.detail.message });
      } else {
        setErrorMsg("");
      }
    };
  ...
  const submit = () => {
    ...
    // Submit to Cyber Arena multiplayer backend
    if (submitDetectiveReport) {
      submitDetectiveReport(timeline, attack, prevention);
    }
