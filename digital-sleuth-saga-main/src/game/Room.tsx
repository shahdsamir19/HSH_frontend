...
export function Room() {
  const { setPhase, status, setStatus, clues, progress, toasts, elapsedMs, addClue } = useGame();
    ...
    <div className="fixed inset-0 bg-black text-white overflow-hidden">
      ...
      <div className="absolute right-3 top-20 z-20 w-64 panel rounded-lg p-3 space-y-3 hidden md:block">
        ...
        <div className="pt-2 border-t border-white/10">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-rose-500 to-amber-500 grid place-items-center font-bold">R</div>
            <div className="text-xs">
              <div className="font-semibold">Rival_42</div>
              <div className="text-white/60 italic">Reading Emails...</div>
            </div>
          </div>
          <div className="mt-2 h-1.5 rounded-full bg-white/10 overflow-hidden">
            <div className="h-full bg-white/40" style={{ width: `${Math.min(95, progress + 5)}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-3 left-3 z-20 panel rounded-lg px-3 py-2 flex items-center gap-3">
        ...
        <div className="flex gap-1">
          {REQUIRED_CLUES.map((c) => (
            <div key={c} title={CLUE_META[c].title}
              className={`w-7 h-7 rounded text-sm grid place-items-center ${clues.has(c) ? "bg-[oklch(0.45_0.2_235)] text-white glow-neon-soft" : "bg-white/10 text-white/30"}`}>
              {CLUE_META[c].emoji}
            </div>
          ))}
        </div>
